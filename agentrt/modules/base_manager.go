...
func (bm *BaseManager[T]) ExecuteGet(ctx context.Context, path string, errorMsg string) (*T, error) {
	...
	if !ok {
		return nil, agentrt.NewError(agentrt.CodeInvalidResponse, errorMsg, nil)
	}
	...
	if dataMap, ok := resp.Data.(map[string]interface{}); ok {
		if success, ok := dataMap["success"]; ok {
			if b, ok := success.(bool); ok && !b {
				return agentrt.NewError(agentrt.CodeInternal, errorMsg, nil)
			}
		}
	}
	return nil
...
func (bm *BaseManager[T]) LogOperation(operation string, resourceID string) {
	agentrt.GetLogger().Printf("[DEBUG] [%s] %s: ID=%s", bm.resourceType, operation, resourceID)
}

func (bm *BaseManager[T]) LogError(operation string, err error) {
	agentrt.GetLogger().Printf("[ERROR] [%s] %s failed: %v", bm.resourceType, operation, err)
}
