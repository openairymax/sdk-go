...
func (mm *MemoryManager) WriteWithOptions(ctx context.Context, content string, layer types.MemoryLayer, metadata map[string]interface{}) (*types.Memory, error) {
	if content == "" {
		return nil, agentrt.NewError(agentrt.CodeMissingParameter, "记忆内容不能为空", nil)
	}
	...
	if !ok {
		return nil, agentrt.NewError(agentrt.CodeInvalidResponse, "记忆写入响应格式异常", nil)
	}
...
func (mm *MemoryManager) Get(ctx context.Context, memoryID string) (*types.Memory, error) {
	if memoryID == "" {
		return nil, agentrt.NewError(agentrt.CodeMissingParameter, "记忆ID不能为空", nil)
	}
	...
	data, ok := utils.ExtractDataMap(resp)
	if !ok {
		return nil, agentrt.NewError(agentrt.CodeInvalidResponse, "记忆详情响应格式异常", nil)
	}
...
func (mm *MemoryManager) Update(ctx context.Context, memoryID string, content string) (*types.Memory, error) {
	...
	data, ok := utils.ExtractDataMap(resp)
	if !ok {
		return nil, agentrt.NewError(agentrt.CodeInvalidResponse, "记忆更新响应格式异常", nil)
	}
...
func (mm *MemoryManager) Delete(ctx context.Context, memoryID string) error {
	if memoryID == "" {
		return agentrt.NewError(agentrt.CodeMissingParameter, "记忆ID不能为空", nil)
	}
...
func parseMemoryList(resp *types.APIResponse) ([]types.Memory, error) {
	data, ok := utils.ExtractDataMap(resp)
	if !ok {
		return nil, agentrt.NewError(agentrt.CodeInvalidResponse, "记忆列表响应格式异常", nil)
	}
...
func parseMemorySearchResult(resp *types.APIResponse, query string, topK int) (*types.MemorySearchResult, error) {
	data, ok := utils.ExtractDataMap(resp)
	if !ok {
		return nil, agentrt.NewError(agentrt.CodeInvalidResponse, "记忆搜索响应格式异常", nil)
	}
