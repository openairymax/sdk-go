...
func (tm *TaskManager) Wait(ctx context.Context, taskID string, timeout time.Duration) (*types.TaskResult, error) {
	...
	for {
		...
		if timeout > 0 && time.Since(start) > timeout {
			return nil, agentrt.NewErrorf(agentrt.CodeTaskTimeout, "任务 %s 超时", taskID)
		}
...
func (tm *TaskManager) GetResult(ctx context.Context, taskID string) (*types.TaskResult, error) {
	...
	if !task.Status.IsTerminal() {
		return nil, agentrt.NewError(agentrt.CodeInvalidParameter, "任务尚未完成", nil)
	}
	return &types.TaskResult{
		ID:     task.ID,
		Status: task.Status,
...
func (tm *TaskManager) Count(ctx context.Context) (int64, error) {
	...
	if err != nil {
		return 0, agentrt.NewError(agentrt.CodeInvalidResponse, err.Error(), nil)
	}
...
func (tm *TaskManager) WaitForAny(ctx context.Context, taskIDs []string, timeout time.Duration) (*types.TaskResult, error) {
	...
	case err := <-errCh:
		return nil, err
	case <-ctx.Done():
		return nil, agentrt.NewError(agentrt.CodeTaskTimeout, "等待任务超时", ctx.Err())
	case <-done:
