...
func TestClient_TimeoutBoundary(t *testing.T) {
	tests := []struct {
		...
		wantErr  bool
		errCode  agentrt.ErrorCode
	}{
		{
			name:    "最小超时_1ms",
			timeout: 1 * time.Millisecond,
			wantErr: true,
			errCode: agentrt.CodeTimeout,
		},
		{
			name:    "正常超时_30s",
		...
		},
		{
			...
			timeout: 0,
			wantErr: true,
			errCode: agentrt.CodeInvalidConfig,
		},
		{
			name:    "负数超时",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				if tt.wantErr && agentrt.IsErrorCode(err, tt.errCode) {
					return
				}
			...
			if tt.wantErr {
				...
				} else if !agentrt.IsErrorCode(err, tt.errCode) && !agentrt.IsErrorCode(err, agentrt.CodeNetworkError) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if err == nil {
					t.Errorf("%s: 期望错误但未返回", tt.name)
				} else if !agentrt.IsErrorCode(err, tt.errCode) {
					t.Errorf("%s: 期望错误码 %s, got %v", tt.name, tt.errCode, err)
				}
				return
			}
...
func TestClient_EmptyEndpoint(t *testing.T) {
	...
	if !agentrt.IsErrorCode(err, agentrt.CodeInvalidEndpoint) {
...
func TestClient_RequestSizeBoundary(t *testing.T) {
	...
	c, err := NewClient(agentrt.WithEndpoint(ts.URL))
	if err != nil {
		t.Fatal(err)
	}
	defer c.Close()
...
func TestClient_ContextCancellation(t *testing.T) {
	...
	if !agentrt.IsErrorCode(err, agentrt.CodeTimeout) {
