...
func DefaultConfig() *Config {
	...
	if v := os.Getenv("AGENTRT_ENDPOINT"); v != "" {
		endpoint = v
	}
	return &Config{
		Endpoint:        endpoint,
...
func NewConfigFromEnv() (*Config, error) {
	...
	if v := os.Getenv("AGENTRT_TIMEOUT"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.Timeout = d
		}
	}
	if v := os.Getenv("AGENTRT_MAX_RETRIES"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n >= 0 {
			cfg.MaxRetries = n
		}
	}
	if v := os.Getenv("AGENTRT_RETRY_DELAY"); v != "" {
		if d, err := time.ParseDuration(v); err == nil {
			cfg.RetryDelay = d
		}
	}
	if v := os.Getenv("AGENTRT_API_KEY"); v != "" {
		cfg.APIKey = v
	}
	if v := os.Getenv("AGENTRT_DEBUG"); v != "" {
		cfg.Debug = v == "true" || v == "1"
	}
	if v := os.Getenv("AGENTRT_LOG_LEVEL"); v != "" {
		cfg.LogLevel = v
	}
	if v := os.Getenv("AGENTRT_MAX_CONNECTIONS"); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n > 0 {
			cfg.MaxConnections = n
		}
	}
	if v := os.Getenv("AGENTRT_USER_AGENT"); v != "" {
		cfg.UserAgent = v
	}
